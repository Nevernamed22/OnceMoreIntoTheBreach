        ...
        private void PostProcess(Projectile bullet, float var)
        {
            if (Owner)
            {
                foreach (Gun gun in Owner.inventory.AllGuns)
                {
                    if (!gun.InfiniteAmmo)
                    {
                        int amountOfMissingBullets = gun.AdjustedMaxAmmo - gun.CurrentAmmo;
                        if (amountOfMissingBullets > 0)
                        {
                            float emptinessPercent = (amountOfMissingBullets / gun.AdjustedMaxAmmo) * 100;
                            bullet.baseData.damage *= (emptinessPercent / 5) + 1;
                        }
                    }
                }
            }
