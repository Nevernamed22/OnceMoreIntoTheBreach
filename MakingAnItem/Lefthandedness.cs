    ...
    public class Lefthandedness : PassiveItem
    {
        ...
        private void PostProcessProj(Projectile bullet, float num)
        {
            if (Owner && bullet && Owner.SpriteFlipped)
            {
                ...
                {
                    EasyTransmogrifyComponent transmog = bullet.gameObject.GetOrAddComponent<EasyTransmogrifyComponent>();
                    EasyTransmogrifyComponent.TransmogData newData = new EasyTransmogrifyComponent.TransmogData()
                    {
