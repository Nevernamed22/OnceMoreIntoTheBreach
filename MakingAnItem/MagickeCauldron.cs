...
namespace NevernamedsItems
{
    class MagickeCauldron : PlayerItem
    {
            ...
            try
            {
                ...
                if (activeEnemies != null)
                {
                    ...
                    foreach (AIActor enemy in enemiesToReRoll)
                    {
                        ...
                        if (user.HasPickupID(325) || user.HasPickupID(569)) guidToTransmogTo = BraveUtility.RandomElement(chaosEnemyPalette);
                        else guidToTransmogTo = BraveUtility.RandomElement(enemylisttouse);
                    ...
                    //Sixth Chamber, Yellow Chamber, High Kaliber, or Kaliber's eye
                    ...
                    if (activeEnemies2 != null)
                    {
                        for (int i = 0; i < activeEnemies2.Count; i++)
                        {
                            ...
                            {
                                if (user.HasPickupID(407) || user.HasPickupID(570) || user.HasPickupID(761) || LastOwner.HasPickupID(Gungeon.Game.Items["nn:kaliber's_eye"].PickupObjectId))
                                {
        ...
        public void ReDetermineEnemyPool()
        {
            ...
            if (!tilesetMatchesKnownTilesets) enemylisttouse = keepEnemyPalette;
        }
